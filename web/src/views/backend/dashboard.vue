                ...
                <el-col :xs="24" :sm="24" :md="24" :lg="6">
                    <el-card class="new-user-card" shadow="hover" :header="t('dashboard.New member')">
                        <div class="new-user-growth">
                            <el-scrollbar>
                                <div class="new-user-item" v-for="user in latestAccounts" :key="user.id">
                                    <img class="new-user-avatar" :src="user.avatar || userSvg" alt="" />
                                    <div class="new-user-base">
                                        <div class="new-user-name">{{ user.nickname }}</div>
                                        <div class="new-user-time">刚刚{{ t('dashboard.Joined us') }}</div>
                                    </div>
...
import userSvg from '/@/assets/login-header.png'
...
const latestAccounts = ref<LatestAccount[]>([])
index().then((res) => {
    state.remark = res.data.remark
    initCountUp(res)
    initUserGrowthChart(res.data.user_trend)
    initFileGrowthChart(res.data.yearly_file_count)
    latestAccounts.value = res.data.latest_accounts
})

const initCountUp = (res: any) => {
    countUpRefs.userRegNumber.value = res.data.account_today
    countUpRefs.fileNumber.value = res.data.file_count
    countUpRefs.usersNumber.value = res.data.account_count
    countUpRefs.addonsNumber.value = res.data.installed_count
}

const initUserGrowthChart = (trendData: number[]) => {
...
const initFileGrowthChart = (yearlyFileCount: Record<string, number> = {}) => {
    const fileGrowthChart = echarts.init(chartRefs.value[1] as HTMLElement)

    const pieData = Object.entries(yearlyFileCount).map(([name, value]) => ({ name, value }))
